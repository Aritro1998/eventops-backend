...
# "redis" (the default) only resolves inside Docker Compose's own
# network, where it's reachable by service name - CI runs the test job
# directly on the runner, not inside a container, so GitHub Actions
# service containers there are only reachable via localhost. Confirmed
# directly: without this override, the test suite doesn't even fail
# cleanly in that environment, it hangs until the job times out.
...
CELERY_BEAT_SCHEDULE = {
    ...
    "cleanup-expired-pending-payment-retries": {
        ...
        "task": "workflows.tasks.cleanup_expired_langgraph_checkpoints_task",
