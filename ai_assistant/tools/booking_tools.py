...
def create_booking(user, request):
    ...
    # A stale or corrupted draft must not silently drop missing seat numbers
    # before it reaches BookingService.
    if len(seat_ids) != len(set(seat_numbers)):
        raise ValueError(
            "The pending booking contains invalid seats. Please choose seats again."
        )
